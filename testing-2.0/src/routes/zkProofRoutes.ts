...
router.post('/submit', async (req: Request, res: Response) => {
  try {
    ...
    console.log('  - publicVerificationPackage:', publicVerificationPackage ? '✅ Present' : '❌ Missing');
    console.log('  - verificationCode:', verificationCode ? '✅ Present' : '❌ Missing');

    // Validate required fields
    if (!electionId || !voterId || !zkProofData || !publicVerificationPackage || !verificationCode) {
      console.log('❌ Validation failed - missing required fields');
      return res.status(400).json({ 
        error: 'Missing required fields',
        required: ['electionId', 'voterId', 'zkProofData', 'publicVerificationPackage', 'verificationCode'],
        received: {
...
router.post('/verify', async (req: Request, res: Response) => {
  try {
    ...
    if (verificationCode) {
      ...
      return res.status(400).json({ error: 'Either verification code or public verification package is required' });
    }
